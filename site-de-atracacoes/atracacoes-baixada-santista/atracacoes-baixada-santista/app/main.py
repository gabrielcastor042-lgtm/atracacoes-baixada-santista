...
@app.get("/debug/embraport_row_raw")
def debug_embraport_row_raw():
    """TEMPORÁRIO: mostra o HTML bruto (cabeçalho + a linha do SAN
    ANTONIO MAERSK 629E) da aba "Todos" da Embraport, posição por
    posição, sem passar pelo nosso parser — pra achar a causa raiz da
    ETB errada por inspeção direta. Tem timeout de 150s (não trava mais
    a requisição pra sempre). Remover depois de usar."""
    ...
    try:
        html = run_in_thread(lambda: scraper._render("Todos"), timeout=150)
    ...
    headers_raw = [
        {"index": i, "nomecoluna": th.get("nomecoluna"), "texto": th.get_text(strip=True)}
    ...
    for tr in body.find_all("tr"):
        ...
        if len(textos) >= 2 and textos[0] == "SAN ANTONIO MAERSK" and textos[1] == "629E":
            linha_alvo = list(enumerate(textos))
            break

    return {
        "total_headers": len(headers_raw),
        "headers_raw": headers_raw,
        "linha_alvo_index_texto": linha_alvo,
    }
